...
└── src/
    ...
    ├── style.css       # 全局样式
    ├── lib/{api,songs,icons,transport}.ts   # transport = 播放传输抽象（见下）
    └── components/
...
`electron/`（桌面壳，不属于 vite 构建）：`main.mjs` 主进程（窗口/托盘/MPRIS daemon 拉起/音频引擎接线）、
`preload.cjs` 桥（窗口控制 + MPRIS + `quaverAudio`）、`native-server.mjs` 打包态静态+/api 服务、
...
- **MPV（默认，`EngineTransport`）**：主进程 spawn `mpv --input-ipc-server=<unix socket> --idle=yes`，
  ...
  **淡入淡出**：不用 `afade`（要掐流时间位置，切歌/暂停场景不好用），而是用 mpv `volume` 属性做振幅包络 ——
  ...
  会把淡出提前收尾再跳，避免先听见 250ms 歌头。时长档位见设置页（关闭/0.15s/0.4s/0.8s，`quaver.fade.v1`）。
...
解码后端偏好 `quaver.decode.v1`：`MPV`（默认）/`Blink`；音频设备偏好 `quaver.audio.device.v1`（mpv `audio-device` 名）。
